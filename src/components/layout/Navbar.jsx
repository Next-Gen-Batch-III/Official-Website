  ...
  return (
    <header className="w-full h-25 flex justify-between items-center gap-10 px-5 lg:px-20 border-b">
      ...
      <nav className="nav flex items-center gap-3 grow justify-end">
        ...
        <ul
          className={`flex flex-col absolute border-t border-b border-t-brand-secondary-orange border-b-brand-secondary-orange top-22 bg-white w-9/10 left-1/20 font-cadt px-10 py-3 pb-6 z-50 ${isMenuOpen === true ? "animate-unroll" : isMenuOpen === false ? "animate-roll-up" : "hidden"} lg:hidden`}
        >
          {navItems.map((item, index) => (
            <li
              key={index}
              className={`flex items-center py-2 border-b border-neutral-400 cursor-pointer transition-colors hover:text-brand-secondary-orange opacity-0 animate-chain-drop`}
              style={{ "--i": index }}
            >
              <NavLink
                to={item === "HOME" ? "/" : `/${item.toLowerCase()}`}
